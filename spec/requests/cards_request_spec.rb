...
RSpec.describe "Check", type: :request do
    describe "POST /api/vi/cards/check" do
        ...
        context "無効な入力の場合" do
            before do
                invalid_cards = {"cards": ["C4441 C2 C3 C124 C12121", "", "C1 C1 C2 C4 C5"]}
            ...
            it "エラーメッセージを返す" do
                json_body = JSON.parse(response.body)
                expect(json_body["error"][0]["card"]).to eq("C4441 C2 C3 C124 C12121")
                expect(json_body["error"][0]["msg"][0]).to eq(" 1 番目のカード指定文字が不正です。(C4441 )5つのカード指定文字を半角スペース区切りで入力してください。（例：'S1 H3 D9 C13 S11')")
                expect(json_body["error"][0]["msg"][1]).to eq(" 4 番目のカード指定文字が不正です。(C124 )5つのカード指定文字を半角スペース区切りで入力してください。（例：'S1 H3 D9 C13 S11')")
                expect(json_body["error"][0]["msg"][2]).to eq(" 5 番目のカード指定文字が不正です。(C12121 )5つのカード指定文字を半角スペース区切りで入力してください。（例：'S1 H3 D9 C13 S11')")
                expect(json_body["error"][1]["card"]).to eq("")
                expect(json_body["error"][1]["msg"][0]).to eq("5つのカード指定文字を半角スペース区切りで入力してください。（例：'S1 H3 D9 C13 S11')")
                expect(json_body["error"][2]["card"]).to eq("C1 C1 C2 C4 C5")
                expect(json_body["error"][2]["msg"][0]).to eq("カードが重複しています。")
            end
        end
        ...
        context "不正なURLの入力" do
            ...
            it "不正なurl入力は404 Not Foundとなる" do
        ...
        context "不正なリクエスト" do
            ...
            it "不正なリクエストは400 Bad Requestとなる" do
